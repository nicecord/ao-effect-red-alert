...
export async function getLatestGameState(): Promise<GameState> {
    ...
    if (data) {
        ...
        for (let [processId, player] of Object.entries(gameState.Players)) {
            player.processId = processId
            player.isFriend = isFriend(processId)
        }
        return gameState
    }

    return { GameMode: 'Loading', Players: {} }
}
